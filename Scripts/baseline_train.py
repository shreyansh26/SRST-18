...
def main(argv):
    options, remainder = getopt.gnu_getopt(argv[1:], 'hp:', ['help','path='])
    HELP, PATH = False, './dataset/'
    ...
    if HELP:
            ...
            print("By default, the program evaluates accuracy")
    ...
    ## Store model
    with open('models/allsrules.pkl', 'wb') as f:
        pickle.dump(allsrules, f)

    with open('models/allprules.pkl', 'wb') as f:
        pickle.dump(allprules, f)

    with open('models/prefbias.pkl', 'wb') as f:
        pickle.dump(prefbias, f)

    with open('models/suffbias.pkl', 'wb') as f:
        pickle.dump(suffbias, f)
