...
with open('word_tag_dict.pickle','rb') as h:
	word_pos = pickle.load(h)	
			...
			for entry in list_permutation:
				...
				if check:
					...
					for idx,word in enumerate(entry):
						# print(temp_sent1[idx])
						if idx!=0:
							
							try:
								prev_tag = word_pos[entry[idx-1]]
								pos_score *= tag_to_tag[prev_tag][word_pos[word]]
...
def find_best_sent(possible_sentences):
			...
			for entry in possible_sentences:
				...
				temp_sent1 = word_tokenize(entry)
				pos_score = 1
				for idx,word in enumerate(temp_sent1):
					# print(temp_sent1[idx])
					if idx!=0:
						
						try:
							prev_tag = word_pos[temp_sent1[idx-1]]
							pos_score *= tag_to_tag[prev_tag][word_pos[word]]
...
with open('real_pred_with_pos.pickle','wb') as h:
